...
namespace GEIS_DC_current_change_ConsoleApp_01
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            // tell how many cycles to measure in GEIS
            Console.WriteLine($"How many cycles to measure in GEIS: {args[19]}");
            int no_cycles = int.Parse(args[19]);
            ...
            // define path to dc, ac currents, and volatge limits, up and down
            string path_to_dc_ac_vol_limits_file = @"C:\GEIS_data\DC_AC_vol_limits_table.txt";

            // inialize dc currents list
            List<float> dc_currents_list = new List<float>();
            // initialize ac currents list
            List<float> ac_currents_list = new List<float>();
            // initialize upper voltage limits list
            List<float> vol_limits_up_list = new List<float>();
            // initialize down voltage limits list
            List<float> vol_limits_down_list = new List<float>();

            // read text file
            using (StreamReader dc_ac_vol_limits_reader = new StreamReader(path_to_dc_ac_vol_limits_file))
            {
                string line;
                while ((line = dc_ac_vol_limits_reader.ReadLine()) != null)
                {
                    // split line
                    string[] values = line.Split(new string[] { " " }, StringSplitOptions.RemoveEmptyEntries);
                    // read dc current value from text file
                    float dc_current_value = float.Parse(values[0]);
                    // read ac current value from text file
                    float ac_current_value = float.Parse(values[1]);
                    // read upper voltage limit value from text file
                    float vol_limit_up_value = float.Parse(values[2]);
                    // read down voltage limit value from text file
                    float vol_limit_down_value = float.Parse(values[3]);
                    // add dc current value to list
                    dc_currents_list.Add(dc_current_value);
                    // add ac current value to list
                    ac_currents_list.Add(ac_current_value);
                    // add upper voltage limit value to list
                    vol_limits_up_list.Add(vol_limit_up_value);
                    // add down volatge limit value to list
                    vol_limits_down_list.Add(vol_limit_down_value);
                }
            }

            // get length of list of dc currents
            int no_dc_currents = dc_currents_list.Count;
            // initialize dc currents array
            double[] dc_currents_array = new double[no_dc_currents];
            // fill dc currents  array
            for (int index_0 = 0; index_0 < no_dc_currents; index_0++)
            {
                dc_currents_array[index_0] = Convert.ToDouble(dc_currents_list[index_0]);
                Console.WriteLine(dc_currents_array[index_0]);
            }

            // get length of list of ac currents
            int no_ac_currents = ac_currents_list.Count;
            // initialize ac currents array
            double[] ac_currents_array = new double[no_ac_currents];
            // fill ac currents array
            for (int index_0 = 0; index_0 < no_ac_currents; index_0++)
            {
                ac_currents_array[index_0] = Convert.ToDouble(ac_currents_list[index_0]);
                Console.WriteLine(ac_currents_array[index_0]);
            }

            // get length of list of upper voltage limits
            int no_vol_limits_up = vol_limits_up_list.Count;
            // initialize upper voltage limits list array
            double[] vol_limits_up_array = new double[no_vol_limits_up];
            // fill upper voltage limits array
            for (int index_0 = 0; index_0 < no_vol_limits_up; index_0++)
            {
                vol_limits_up_array[index_0] = Convert.ToDouble(vol_limits_up_list[index_0]);
                Console.WriteLine(vol_limits_up_array[index_0]);
            }

            // get length of list of down voltage limits
            int no_vol_limits_down = vol_limits_down_list.Count;
            // initialize down voltage limits array
            double[] vol_limits_down_array = new double[no_vol_limits_down];
            // fill down voltage limits array
            for (int index_0 = 0; index_0 < no_vol_limits_down; index_0++)
            {
                vol_limits_down_array[index_0] = Convert.ToDouble(vol_limits_down_list[index_0]);
                Console.WriteLine(vol_limits_down_array[index_0]);
            }
            ...
            if (vol_current < vol_limits_up_array[0])
            {
                ...
                my_instrument.Ei.Setpoint = dc_currents_array[0];
            }
            else
            {
                // increnment counter for charging and discharging events
                ...
                // set offset/DC curent
                my_instrument.Ei.Setpoint = (-1.0f) * dc_currents_array[0];
            }
            ...
            while (counter_events < max_events)
            {
                ...
                if (direction == "charging" && vol_current < vol_limits_up_array[0])
                {
                ...
                else if (direction == "charging" && vol_current >= vol_limits_up_array[0])
                {
                ...
                else if (direction == "discharging" && vol_current >= vol_limits_up_array[0])
                {
                    // increment counter for charging and discharging events
                    counter_events++;
                    Console.WriteLine("Do battery discharging.");
            ...
            // initialize counter for charging
            int counter_charging = 0;
            // initialize counter for discharging
            int counter_discharging = 0;
            ...
            // empty counter for charging and discharging events
            counter_events = 0;
            // initialize counter for dc currents
            int counter_dc_currents = 0;
            // main while loop
            while ((direction == "charging" || direction == "discharging") && (counter_dc_currents < max_no_cycles) && (counter_dc_currents < no_dc_currents))
            {
                // initialize counter
                int counter = 0;
                while ((counter_charging <= no_cycles) && (counter_discharging < no_cycles))
                {
                    if (direction == "charging")
                    {
                        // set offset/DC curent
                        my_instrument.Ei.Setpoint = dc_currents_array[counter_dc_currents];
                        ...
                        // condition
                        if (vol_current < vol_limits_up_array[counter_dc_currents])
                        {
                            for (int index_0 = 0; index_0 < no_freq_points; index_0++)
                            {
                                if (index_0 == 0)
                                {
                                    ...
                                    // set sinus amplitude using FRA module
                                    my_instrument.Fra.Amplitude = ac_currents_array[counter_dc_currents];
                                    // Switch on the FRA
                                    ...
                                    // filename
                                    string vol_filename = "voltage_data";
                                    // initialize current number for dc currents
                                    int current_no_dc_currents = counter_dc_currents + 1;
                                    // convert current number for dc currents to string
                                    string current_no_dc_currents_str = current_no_dc_currents.ToString().PadLeft(6, '0');
                                    // build full file name
                                    string vol_filename_full = vol_filename + "_" + current_no_dc_currents_str + ".txt";
                                    // build full path to file
                                    string vol_full_path = Path.Combine(root_folder, vol_filename_full);

                                    // write current voltage to text file
                                    using (StreamWriter volw = System.IO.File.AppendText(vol_full_path))
                                    {
                                    ...
                                    // save time data
                                    // root folder
                                    root_folder = @"c:\GEIS_data";
                                    // filename
                                    string time_filename = "time_data";
                                    // initialize current number for dc currents
                                    current_no_dc_currents = counter_dc_currents + 1;
                                    // convert current number for dc currents to string
                                    current_no_dc_currents_str = current_no_dc_currents.ToString().PadLeft(6, '0');
                                    // build full file name
                                    string time_filename_full = time_filename + "_" + current_no_dc_currents_str + ".txt";
                                    // build full path to file
                                    string time_full_path = Path.Combine(root_folder, time_filename_full);

                                    // write timestamp to text file
                                    using (StreamWriter timew = System.IO.File.AppendText(time_full_path))
                                    {
                                ...
                                else
                                {
                                    ...
                                    if (index_0 == (no_freq_points - 1))
                                    {
                                        ...
                                        // build full file name
                                        string filename_full = filename + "_" + current_no_dc_currents_str + "_" + counter_str + ".txt";
                                        // build full path to file
                        ...
                        else
                        {
                            ...
                            if (counter_events >= max_events)
                            {
                                ...
                                // save charge/discharge time data
                                // root folder
                                string root_folder = @"c:\GEIS_data";
                                // filename
                                string time_charge_discharge_filename = "time_charge_discharge";
                                // initialize current number for dc currents
                                int current_no_dc_currents = counter_dc_currents + 1;
                                // convert current number for dc currents to string
                                string current_no_dc_currents_str = current_no_dc_currents.ToString().PadLeft(6, '0');
                                // build full file name
                                string time_charge_discharge_filename_full = time_charge_discharge_filename + "_" + current_no_dc_currents_str + ".txt";
                                // build full path to file
                                string time_charge_discharge_full_path = Path.Combine(root_folder, time_charge_discharge_filename_full);

                                // write timestamp to text file
                                using (StreamWriter timeswitch = System.IO.File.AppendText(time_charge_discharge_full_path))
                                {
                                    string timestamp = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                    ...
                    {
                        // set offset/DC curent
                        my_instrument.Ei.Setpoint = (-1.0f) * dc_currents_array[counter_dc_currents];
                        // wait time after setting DC current
                        System.Threading.Thread.Sleep(wait_time_DC_current);
                        ...
                        // condition
                        if (vol_current > vol_limits_down_array[counter_dc_currents])
                        {
                            for (int index_0 = 0; index_0 < no_freq_points; index_0++)
                            {
                                if (index_0 == 0)
                        ...
                        else
                        {
                            ...
                            if (counter_events >= max_events)
                            {
                                ...
                                // empty counter for charging and discharging events
                                counter_events = 0;
                                // increase counter for discharging
                                counter_discharging++;
                            }
                            else if (counter_events < max_events && counter_events >= 0)
                            {
                    ...
                    string path_to_stop_file_insider = @"c:\GEIS_data\stop.txt";
                    // open with shared read/write access
                    FileStream fs_insider = new FileStream(path_to_stop_file_insider, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite);
                    // create stream reader object
                    using (StreamReader sr = new StreamReader(fs_insider))
                    {
                        // read first line
                ...
                // clear counters for charging and discharging
                counter_charging = 0;
                counter_discharging = 0;
                // increment counter for dc currents
                counter_dc_currents++;
